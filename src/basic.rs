...
impl<L: for <'q> LedgerTrait<'q> + Clone> Bank for Accountant<L> {
    fn deposit(&mut self, client: Client, tx_id: TxId, amount: Decimal) -> Result<(), TxError> {
        ...
        self.ledger.put_account(client, match opt_acc {
            Some(acc) => Account {
    ...
    fn withdrawal(&mut self, client: Client, tx_id: TxId, amount: Decimal) -> Result<(), TxError> {
        ...
        match opt_acc {
            ...
            Some(acc) if acc.locked =>
                Err(TxError::Rejected("account is locked".to_string())),
...
impl<L: for <'q> LedgerTrait<'q> + Clone> Accountant<L> {
    fn get_and_check_tx_acc(&self, client: Client, tx_id: TxId, tx_state: TxState) -> Result<(Transaction,Account), TxError> {
        let opt_tx = self.ledger.get_transaction(tx_id)?;
        let opt_acc = self.ledger.get_account(client)?;
        match (opt_tx, opt_acc) {
            (None, _) =>
                Err(TxError::Rejected("deposit transaction does not exist".to_string())),
            (_, None) =>
                Err(TxError::Rejected("disputed account does not exist".to_string())),
            (Some(tx), _) if tx.client != client =>
                Err(TxError::Rejected("malicious transaction, wrong client".to_string())),
            (Some(tx), _) if tx.state != tx_state =>
                match tx_state {
            ...
            // TODO: unknown case
            (_, Some(acc)) if acc.locked =>
                Err(TxError::Rejected("account is locked".to_string())),
            // TODO: unknown case; can it be possible?!
            (Some(tx), Some(acc)) if tx.state == TxState::Disputed && tx.amount > acc.held =>
                Err(TxError::Rejected("insufficient funds for resolve/chargeback".to_string())),
            // TODO: unknown case
            (Some(tx), Some(acc)) if tx.state == TxState::Committed && tx.amount > acc.available =>
                Err(TxError::Rejected("insufficient funds for dispute".to_string())),
